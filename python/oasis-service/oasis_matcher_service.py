"""
O.A.S.I.S. Protocol Matcher Service (Flask)
"""
print("[OASIS] Starting Python Service...", flush=True)
...
def build_index(protocols, model):
    ...
    print(f"[OASIS] Embedding {len(sentences)} scenarios...")
...
model = None
index = None
sentence_to_protocol = None

def initialize():
    global model, index, sentence_to_protocol
    print("[OASIS] Loading model: all-MiniLM-L6-v2...", flush=True)
    model = SentenceTransformer("all-MiniLM-L6-v2")
    index, sentence_to_protocol = build_index(PROTOCOLS, model)
    print(f"[OASIS] Ready. Index size: {index.ntotal}", flush=True)
...
def match_protocol():
    ...
    result = {
        "match": is_match,
        "score": score,
        "protocol_id": matched_proto["id"] if is_match else "TRIAGE",
        "text": matched_proto["text"] if is_match else TRIAGE_PROTOCOL,
        "triage": not is_match
    }
    
    # Log concise result
    if is_match:
        print(f"[MATCH] {matched_proto['id']} ({score:.3f}): '{user_input}'")
    else:
        print(f"[TRIAGE] ({score:.3f}): '{user_input}'")
