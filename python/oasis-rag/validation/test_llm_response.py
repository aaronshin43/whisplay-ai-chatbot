...
# Each entry: (match_signals, prepend_text, append_text, exclude_if_any_signals)
# prepend_text is prepended to context; append_text (if set) is appended after.
# exclude_if_any_signals: skip this entry when any of these signals also match.
_SIGNAL_TABLE: list[tuple[list[str], str, str, list[str]]] = [
    (_CARDIAC_ARREST_SIGNALS,
    ...
    (_BURN_SIGNALS,
     ...
     "", _EYE_CHEMICAL_SIGNALS),  # skip if eye chemical query takes priority
    (_CHOKING_SIGNALS,
...
def _inject_signals(query: str, context: str) -> str:
    """Prepend (and optionally append) emergency protocol text based on query signals."""
    q_lower = query.lower()
    for signals, prepend, append, exclude in _SIGNAL_TABLE:
        if any(sig in q_lower for sig in signals):
            if not (exclude and any(sig in q_lower for sig in exclude)):
...
def call_llm(context: str, query: str, model: str = MODEL) -> str:
    context = _inject_signals(query, context)
